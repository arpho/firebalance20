...
export class DistanceSorterPipe implements PipeTransform {
  ...
  transform(value: any, location) {
    console.log('distance sorter',location)
    if(value &&location)
    return value.sort((a:ProviderModel,b:ProviderModel)=>{return this.distance(a.latitudine, a.longitudine, location.latitude, location.longitude)-
